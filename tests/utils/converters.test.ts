...
describe('Conversion rate tests', () => {
	...
	test.each([
		...
		expect(getConversionRate(rates, fromCurrency, 'USD')).toBeCloseTo(expectedRate, 1)
	})
	...
	test.each([
		...
		expect(getConversionRate(rates, fromCurrency, toCurrency)).toBeCloseTo(expectedRate, 1)
	})
})
