...
import type { AddItemToBudgetRequest, BudgetWithItems, Item } from "./api";
...
interface ReducerOptions {
	addItemToBudgetCallback: (budget_id: string, request: AddItemToBudgetRequest) => Promise<string>;
	deleteItemFromBudgetCallback: (budget_id: string, item_id: string) => Promise<void>;
	updateItemCallback: (
		budget_id: string,
		item_id: string,
	...
	return async (state: State, action: Action) => {
		switch (action.type) {
			...
			case "ADD INCOME": {
				...
				const item_id = await options.addItemToBudgetCallback(action.budget_id, action.item);
			...
			case "REMOVE ITEM":
				...
				await options.deleteItemFromBudgetCallback(action.budget_id, action.item_id);
			...
			case "EDIT ITEM":
				...
				await options.updateItemCallback(state.budget.id, action.item_id, action.item);
