...
import type { User } from "utils/githubAuth";

describe("LoginButton", () => {
	it("Login button with link", () => {
		const url = "some-random-url";
		cy.mount(<LoginButton authorizeUrl={url} />);
		cy.get("a").should("have.attr", "href", url).and("have.class", "btn").and("have.text", "Login");
	});
});
...
describe("LoginState", () => {
	...
	it("Without authorized user should just show a login button", () => {
		...
		cy.mount(<LoginState user={null} authorizeUrl={url} logoutUrl={logoutUrl} />);
		cy.get("a")
	...
	it("With an authorized user", () => {
		...
		cy.mount(<LoginState user={user} authorizeUrl={url} logoutUrl={logoutUrl} />);
