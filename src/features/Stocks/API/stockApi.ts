...
export function useTrackStockCallback(): (symbol: string) => Promise<Response | undefined> {
	return useApiCall(`${apiUrlWithPath}/stock/tracked`, {
		method: 'POST',
	})
}

export function useAddStockLotCallback(): (lot: AddStockLotRequest) => Promise<string> {
...
export function useDeleteStockLotCallback(): (id: string) => Promise<Response | undefined> {
	const handler = useApiWithUrlCall()

	return useCallback(
		(id: string) => handler(`${apiUrlWithPath}/stock/lot/${id}`, { method: 'DELETE' }),
		[handler],
	)
}
